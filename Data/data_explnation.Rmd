...
__Interactions:__ This include `transect`, `visitation` and `out` data. Data can be used pooled (1472 interactions ; 5706 visits) for general question abouts trait matching or splited by site (`Site_ID`) and `Round`.
...
head(pol_tr)
```
__NOTE:__ Most IT (intertegular length) for bees are actually measured. For floral visitors without wings we measured torax width. When species is unknown (Genus sp.) we calculated the mean of the genus or family. Tongue length is measured only for a few individuals. Bees tongue length is estimated following Cariveau et al 2016. For families without an actual tongue the value is set to zero. For other species, tongue length is estimated to covary with allometry using same equation as for colletidaes (the shortest tongue bee family).
...
```{r}
head(int)
unique(int[,3:4]) #why NA NA
unique(int$Plant_gen_sp[which(!int$Plant_gen_sp %in% plant_tr$Plant_gen_sp)]) #FUCK!
